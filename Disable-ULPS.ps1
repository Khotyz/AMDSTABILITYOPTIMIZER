...
function Get-Text {
    ...
    # Captura o idioma direto do SO e da instalação do Windows
    $installedCulture = [System.Globalization.CultureInfo]::InstalledUICulture.TwoLetterISOLanguageName
    $userCulture      = [System.Globalization.CultureInfo]::CurrentUserCulture.TwoLetterISOLanguageName
    $uiCulture        = (Get-UICulture).TwoLetterISOLanguageName

    $lang = 'en-US'

    if ($installedCulture -eq 'pt' -or $userCulture -eq 'pt' -or $uiCulture -eq 'pt') {
        $lang = 'pt-BR'
    } elseif ($installedCulture -eq 'es' -or $userCulture -eq 'es' -or $uiCulture -eq 'es') {
        $lang = 'es-ES'
    } elseif ($installedCulture -eq 'zh' -or $userCulture -eq 'zh' -or $uiCulture -eq 'zh') {
        $lang = 'zh-CN'
    }
